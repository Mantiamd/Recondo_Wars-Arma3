...
- **Foot Patrols** - Spawn randomized foot patrol groups with configurable routes. Patrol members quietly talk in Vietnamese when players are within 100m (toggleable, on by default). Optional Headless Client transfer
- **Path Patrols** - Create patrols that follow specific marker paths. Configurable patrol behaviour (default SAFE); patrols automatically switch to COMBAT when they know about an enemy and resume relaxed patrolling once the contact is over. Patrol members quietly talk in Vietnamese when players are within 100m (toggleable, on by default). Optional Headless Client transfer
- **Add AI Crew** - Dynamically add crew members to player vehicles
- **Static Defense Randomized** - Spawn randomized static weapon positions. Optional Headless Client transfer. Optional "Quick Reaction AA": instead of spawning at mission start, the randomly-selected sites all spawn simultaneously the first time any Detector Side AI unit within Detection Radius of a selected site spots (knowsAbout ≥ threshold, default 1.5) a Target Side unit. Uses Arma's built-in stealth model - line of sight, stance, cover and noise all matter. Max-altitude gate applies to both detectors and targets so high-flying jets don't arm the layout. Per-site safety check on trip (default 200m): any individual site with a Target Side unit inside the safety radius is silently skipped (lost, not retried) so AA never spawns in front of the enemy. QRAA bypasses persistence and HC transfer while active
...
- **Recon Points** - Award points for reconnaissance activities. Kill rewards and non-objective awards (e.g. Hanoi Hannah loudspeaker disable) still show the on-screen RP toast; objective-completion awards (Destroy / Hub & Subs / Jammer / HVT / Hostages / Photographs / Intel & POW turn-ins) are silent and only reflected on the Intel Board
...
- **Wave Attack** - Marker-triggered multi-wave assaults from configurable compass bearings, with whistle audio cues on all attacking waves. Configurable initial delay (default 10s) between the trigger firing and the first wave spawning; once triggered, all waves run to completion. Optional "Enable Radio Chatter" plays quiet Vietnamese radio chatter (40 sound variations, audible to ~150m) from attacking groups while within 100m of a player - immediately on first close contact, then on a jittered ~60s cadence, alongside the whistles. Optional Headless Client transfer. Optional "Chargers" second tier: charger groups with their own classname pool (pick units with bayonets fixed), group size (min/max), wave count, and countdown run in parallel from the same trigger moment - they sprint straight at players (CARELESS, full speed, no fleeing, force walk removed) and use LAMBS Danger's rush system when it is loaded (plain full-speed assault to the marker without it). Once within 100m of players, a random charger screams a charging war cry (30 sound variations, audible to ~300m) roughly every 15 seconds - always on when chargers are enabled. Chargers always stay on the server (no HC transfer)
...
- **Camps Random** - Randomized camp placement with composition support. Camp sentries quietly talk in Vietnamese when players are within 100m (toggleable, on by default)
