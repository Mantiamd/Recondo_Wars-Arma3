/*
    Recondo_fnc_createObjDestroySmellTrigger
    Creates a proximity trigger for smell hints at objective locations
...
if (isNil "_settings" || _marker == "") exitWith {
    diag_log "[RECONDO_OBJDESTROY] ERROR: Invalid parameters for createSmellTrigger";
};
...
if (count _smellHintMessages == 0) exitWith {
    if (_debugLogging) then {
        diag_log format ["[RECONDO_OBJDESTROY] No smell messages configured, skipping smell trigger at %1", _marker];
    };
};
...
_trigger setVariable ["RECONDO_OBJDESTROY_marker", _marker];
_trigger setVariable ["RECONDO_OBJDESTROY_smellMessages", _smellHintMessages];
_trigger setVariable ["RECONDO_OBJDESTROY_debugLogging", _debugLogging];

// Trigger condition and activation - each player only gets the hint once per marker
_trigger setTriggerStatements [
    "
        private _marker = thisTrigger getVariable 'RECONDO_OBJDESTROY_marker';
        private _messages = thisTrigger getVariable 'RECONDO_OBJDESTROY_smellMessages';
        private _debug = thisTrigger getVariable 'RECONDO_OBJDESTROY_debugLogging';
        {
            if (isPlayer _x) then {
                private _playerUID = getPlayerUID _x;
                private _smellKey = format ['%1_%2', _marker, _playerUID];
                if !(_smellKey in RECONDO_OBJDESTROY_SMELL_TRIGGERED) then {
                    RECONDO_OBJDESTROY_SMELL_TRIGGERED pushBack _smellKey;
                    private _message = selectRandom _messages;
                    [_message] remoteExec ['Recondo_fnc_showObjDestroySmellHint', _x];
                    if (_debug) then {
                        diag_log format ['[RECONDO_OBJDESTROY] Smell hint triggered for %1 at %2', name _x, _marker];
                    };
                };
            };
        } forEach thisList;
...
if (_debugLogging) then {
    diag_log format ["[RECONDO_OBJDESTROY] Created smell trigger at %1, radius: %2m, messages: %3", 
