/*
    Recondo_fnc_updateHostageNightLights
    Background loop that updates night lights for Hostage composition buildings
    
    Description:
        Periodically checks time of day (sunOrMoon) and enables/disables
        lights in all registered Hostage composition buildings.
...
if (!RECONDO_HOSTAGE_NIGHT_LIGHTS_ENABLED) exitWith {
    diag_log "[RECONDO_HOSTAGE] Night lights disabled - not starting loop";
};

diag_log "[RECONDO_HOSTAGE] Starting night lights update loop";
...
[{
    ...
    if (count RECONDO_HOSTAGE_NIGHT_LIGHT_BUILDINGS == 0) exitWith {};
    ...
    
    {
        ...
        if (!isNull _building) then {
            private _existingLight = _building getVariable ["RECONDO_HOSTAGE_Light", objNull];
    ...
    } forEach RECONDO_HOSTAGE_NIGHT_LIGHT_BUILDINGS;
