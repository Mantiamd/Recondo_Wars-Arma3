/*
    Recondo_fnc_createHVTTrigger
    Creates a proximity trigger for HVT location (outer ring - composition)
    
    Description:
        Creates a trigger that spawns the composition when players approach.
        This is the outer ring trigger - spawns composition only.
...
if (isNil "_settings" || _marker == "") exitWith {
    diag_log "[RECONDO_HVT] ERROR: Invalid parameters for createHVTTrigger";
};
...
_trigger setVariable ["RECONDO_HVT_isHVTLocation", _isHVTLocation];
...
_trigger setTriggerStatements [
    ...
    "
        ...
        [thisTrigger, _settings, _marker, _composition, _isHVTLocation, _isModPath] call Recondo_fnc_handleHVTTriggerActivation;
    ",
    ""
];
...
if (_debugLogging) then {
    ...
    diag_log format ["[RECONDO_HVT] Created %1 trigger at %2, radius: %3m", _locType, _marker, _compositionTriggerRadius];
};
