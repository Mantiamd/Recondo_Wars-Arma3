/*
    Recondo_fnc_updateHVTNightLights
    Background loop that updates night lights for HVT composition buildings
    
    Description:
        Periodically checks time of day (sunOrMoon) and enables/disables
        lights in all registered HVT composition buildings.
...
if (!RECONDO_HVT_NIGHT_LIGHTS_ENABLED) exitWith {
    diag_log "[RECONDO_HVT] Night lights disabled - not starting loop";
};

diag_log "[RECONDO_HVT] Starting night lights update loop";
...
[{
    ...
    if (count RECONDO_HVT_NIGHT_LIGHT_BUILDINGS == 0) exitWith {};
    ...
    
    {
        ...
        if (!isNull _building) then {
            private _existingLight = _building getVariable ["RECONDO_HVT_Light", objNull];
            ...
            if (_shouldLightsBeOn) then {
                ...
                if (isNull _existingLight) then {
                    ...
                    // Store reference
                    _building setVariable ["RECONDO_HVT_Light", _light, true];
                    
                    // Track globally
                    RECONDO_HVT_ACTIVE_LIGHTS pushBack _light;
                };
            } else {
                // Remove light if exists (daytime)
                if (!isNull _existingLight) then {
                    // Remove from global tracking
                    RECONDO_HVT_ACTIVE_LIGHTS = RECONDO_HVT_ACTIVE_LIGHTS - [_existingLight];
                    
                    deleteVehicle _existingLight;
                    _building setVariable ["RECONDO_HVT_Light", objNull, true];
                };
            };
        };
    } forEach RECONDO_HVT_NIGHT_LIGHT_BUILDINGS;
