/*
    Recondo_fnc_rpShowObjectiveStatus
    Independent objective status display for Roleplayer Source module.
    Mirrors the Terminal's objective status but operates independently.
*/
...
if (_debug) then {
    diag_log format ["[RECONDO_RP_SOURCE] Displayed objective status: %1 lines", count _statusLines];
};
